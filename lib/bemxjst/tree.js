...
AddMatch.prototype.mixWrapBody = function mixWrapBody() {
  var refs = this.refs;
  var applyCtx = refs.applyCtx;
  var apply = refs.apply;
  ...
  return function addMixAdaptor() {
    var ret = apply('mix');
    if (!Array.isArray(ret)) ret = [ ret ];
    return ret.concat(applyCtx(body.call(this, this, this.ctx)));
  };
};
...
AddMatch.prototype.attrsWrapBody = function attrsWrapBody(body) {
  ...
  if (typeof body !== 'function') {
    return function inlineAddAdaptor() {
      return utils.extend(apply('attrs') || {}, body);
    };
  }

  return function addAdaptor() {
...
Tree.methods = [
  ...
  'elemMod', 'def', 'tag', 'addAttrs', 'attrs', 'cls', 'js',
  'bem', 'addMix', 'mix', 'content', 'replace', 'extend', 'oninit',
